...
頭文字の代わりに、はてなの配信先から取得した favicon を 20 pt で表示する。取得前・取得失敗時は同じ大きさの地球アイコンを置き、行の幅や高さを変えない。
結果の行高は 160 pt に固定する。タイトル 38 pt、ドメインと日付 16 pt、コメント 36 pt、タグ 22 pt の領域を確保し、空欄でも詰めない。タイトルとコメントはそれぞれ最大 2 行とする。
`ScrollView` 内には全件分の軽い固定高の枠を置き、スクロール領域の高さを件数 × 160 pt で最初に確定する。内容と favicon の読み込みは画面内と前後 1 画面分に限定する。
描画範囲の更新は [onGeometryChange](https://developer.apple.com/documentation/SwiftUI/View/onGeometryChange%28for%3Aof%3Aaction%3A%29) で行い、スクロールの各ピクセルではなく行の境界を越えたときだけ反映する。未描画の行も位置と ID を持つため、キーボードで画面外の候補を選んでもその行へ移動できる。
...
会話を始める前は、時期と話題・やりたいこと・覚えている内容・名前を忘れたツールの4種類の具体的な検索例を表示する。「先月見た、React の新機能を解説した記事」のように、対象まで含む独立した例文にする。
例を選ぶと入力欄に入れてフォーカスを移し、書き換えられるようにする。例の選択だけでは送信せず、送信ボタンか Command + Enter で開始する。入力欄にも時期と話題を含む例を表示する。
この案内は [Google PAIR の Mental Models](https://pair.withgoogle.com/guidebook-v2/chapter/mental-models/) にある、用途を具体例で伝え、小さく試せる操作を用意する考え方を参考にする。
送信時点の検索語と候補の ID を渡す。
...
favicon は検索結果の行を表示するときに別の非同期処理で読み込み、検索処理は画像を待たない。
[はてなの公式ドキュメント](https://developer.hatena.ne.jp/ja/documents/bookmark/apis/oembed/)に載っている配信先を使い、[はてな上の埋め込みコード](https://b.hatena.ne.jp/entry/4699493974694001410/comment/efcl)と実際の応答で確認した `/64?url=...` から 64 px の画像を取得する。
送信する URL はホスト名から作る HTTPS のルート URL とし、記事のパス・検索条件・フラグメント・ユーザー情報は含めない。同じサイトの同時取得をまとめ、アプリの保存先の `favicons` に画像と次回取得時刻を一度に保存する。
保存画像を先に表示し、7 日経過後に裏で更新する。失敗しても古い画像を保持し、画像がない場合を含めて 1 時間は再試行しない。メモリには最大 256 サイトを保持し、再起動後は保存ファイルを再利用する。
